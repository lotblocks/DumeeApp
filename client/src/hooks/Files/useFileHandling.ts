...
import type { EndpointFileConfig, TEndpointsConfig, TError } from 'dumee-data-provider';
  ...
  const displayToast = useCallback(() => {
    if (errors.length > 1) {
      ...
      const errorList = Array.from(new Set(errors))
        .map((e, i) => `${i > 0 ? 'â€¢ ' : ''}${localize(e as TranslationKeys) || e}\n`)
        .join('');
      showToast({
        message: errorList,
        status: 'error',
        ...
        if (heicProcessedFile.type.startsWith('image/')) {
          try {
            ...
            if (resizeResult.resized && resizeResult.result) {
              ...
              showToast({
                message: `Image resized: ${originalSizeMB}MB â†’ ${newSizeMB}MB (${savedPercent}% smaller)`,
                status: 'success',
                duration: 3000,
              });
            }
