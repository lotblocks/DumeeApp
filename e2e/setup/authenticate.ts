...
async function authenticate(config: FullConfig, user: User) {
  console.log('ðŸ¤–: global setup has been started');
  ...
  try {
    ...
    console.log('ðŸ¤–: ðŸ—  authenticating user:', user.email);

    if (!baseURL) {
      throw new Error('ðŸ¤–: baseURL is not defined');
    }
    ...
    console.log('ðŸ¤–: âœ”ï¸  localStorage: set Nav as Visible', storageState);
    ...
    try {
      ...
      if (userExists) {
        console.log('ðŸ¤–: ðŸš¨  user already exists');
        await cleanupUser(user);
        await page.goto(baseURL, { timeout });
        await register(page, user);
      } else {
        throw new Error('ðŸ¤–: ðŸš¨  user failed to register');
      }
    }
    console.log('ðŸ¤–: âœ”ï¸  user successfully registered');

    // Logout
    // await logout(page);
    // await page.waitForURL(`${baseURL}/login`, { timeout });
    // console.log('ðŸ¤–: âœ”ï¸  user successfully logged out');

    await login(page, user);
    await page.waitForURL(`${baseURL}/c/new`, { timeout });
    console.log('ðŸ¤–: âœ”ï¸  user successfully authenticated');

    await page.context().storageState({ path: storageState as string });
    console.log('ðŸ¤–: âœ”ï¸  authentication state successfully saved in', storageState);
    // await browser.close();
    // console.log('ðŸ¤–: global setup has been finished');
  } finally {
    await browser.close();
    console.log('ðŸ¤–: global setup has been finished');
  }
