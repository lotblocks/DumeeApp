...
import type { TUser } from 'dumee-data-provider';
...
describe('Environment Variable Extraction (MCP)', () => {
  ...
  describe('processMCPEnv', () => {
    ...
    it('should ensure different users with same starting config get separate values', () => {
      ...
      expect(baseConfig.headers?.['User-Id']).toBe('{{Dumee_USER_ID}}');
    ...
    it('should process dynamic user fields in headers', () => {
      ...
      const obj: MCPOptions = {
        ...
        url: 'https://example.com',
        headers: {
          'User-Email': '{{Dumee_USER_EMAIL}}',
          'User-Name': '{{Dumee_USER_USERNAME}}',
          OpenID: '{{Dumee_USER_OPENIDID}}',
          'Google-ID': '{{Dumee_USER_GOOGLEID}}',
          'Email-Verified': '{{Dumee_USER_EMAILVERIFIED}}',
          'User-Role': '{{Dumee_USER_ROLE}}',
          'Content-Type': 'application/json',
        },
      };
    ...
    it('should process user fields in env variables', () => {
      ...
      const obj: MCPOptions = {
        ...
        args: ['server.js'],
        env: {
          USER_EMAIL: '{{Dumee_USER_EMAIL}}',
          LDAP_ID: '{{Dumee_USER_LDAPID}}',
          API_KEY: '${TEST_API_KEY}',
        },
      };
    ...
    it('should process user fields in URL', () => {
      ...
      const obj: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api/{{Dumee_USER_USERNAME}}/stream',
      };
    ...
    it('should handle boolean user fields', () => {
      ...
      const obj: MCPOptions = {
        ...
        headers: {
          'Email-Verified': '{{Dumee_USER_EMAILVERIFIED}}',
          'Two-Factor': '{{Dumee_USER_TWOFACTORENABLED}}',
          'Terms-Accepted': '{{Dumee_USER_TERMSACCEPTED}}',
        },
      };
    ...
    it('should not process sensitive fields like password', () => {
      ...
      const obj: MCPOptions = {
        ...
        headers: {
          'User-Email': '{{Dumee_USER_EMAIL}}',
          'User-Password': '{{Dumee_USER_PASSWORD}}', // This should not be processed
        },
      };
      ...
      expect('headers' in result && result.headers).toEqual({
        'User-Email': 'test@example.com',
        'User-Password': '{{Dumee_USER_PASSWORD}}', // Unchanged
      });
    });
    ...
    it('should handle multiple occurrences of the same placeholder', () => {
      ...
      const obj: MCPOptions = {
        ...
        url: 'https://example.com',
        headers: {
          'Primary-Email': '{{Dumee_USER_EMAIL}}',
          'Secondary-Email': '{{Dumee_USER_EMAIL}}',
          'Backup-Email': '{{Dumee_USER_EMAIL}}',
        },
      };
    ...
    it('should support both id and _id properties for Dumee_USER_ID', () => {
      ...
      expect('headers' in result2 && result2.headers?.['User-Id']).toBe('{{Dumee_USER_ID}}');
    ...
    it('should process customUserVars in URL field', () => {
      ...
      const obj: MCPOptions = {
        type: 'websocket',
        url: 'wss://example.com/{{TENANT_ID}}/api/{{API_VERSION}}?user={{Dumee_USER_ID}}&key=${TEST_API_KEY}',
      };
    ...
    it('should prioritize customUserVars over user fields and system env vars if placeholders are the same (though not recommended)', () => {
      ...
      process.env.Dumee_USER_EMAIL = 'system-email-should-be-overridden';
      const user = createTestUser({ email: 'user-email-should-be-overridden' });
      const customUserVars = {
        Dumee_USER_EMAIL: 'custom-email-wins',
      };
      const obj: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api',
        headers: {
          'Test-Email': '{{Dumee_USER_EMAIL}}', // Placeholder that could match custom, user, or system
        },
      };
      ...
      // Clean up env var
      delete process.env.Dumee_USER_EMAIL;
    });
    ...
    it('should handle placeholders with no matching customUserVars (falling back to user/system vars)', () => {
      ...
      const obj: MCPOptions = {
        ...
        url: 'https://example.com/api',
        headers: {
          'User-Email-Header': '{{Dumee_USER_EMAIL}}', // Should use user.email
    ...
    it('should correctly process a mix of all variable types', () => {
      ...
      const obj = {
        type: 'streamable-http' as const,
        url: 'https://{{CUSTOM_ENDPOINT_ID}}.example.com/users/{{Dumee_USER_USERNAME}}',
        headers: {
        ...
        env: {
          ...
          USER_HOME_DIR: '/home/{{Dumee_USER_USERNAME}}',
    ...
    it('should process GitHub MCP server configuration with PAT_TOKEN placeholder', () => {
      ...
      // Simulate the GitHub MCP server configuration from Dumee.yaml
      const obj: MCPOptions = {
        type: 'streamable-http',
