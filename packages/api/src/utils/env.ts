import { extractEnvVariable } from 'dumee-data-provider';
import type { TUser, MCPOptions } from 'dumee-data-provider';
...
function processUserPlaceholders(value: string, user?: TUser): string {
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{Dumee_USER_${field.toUpperCase()}}}`;
...
function processSingleValue({
  ...
  // 2. Replace user field placeholders (e.g., {{Dumee_USER_EMAIL}}, {{Dumee_USER_ID}})
