...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      ...
      'User-Name': '{{Dumee_USER_NAME}}',
      'User-Username': '{{Dumee_USER_USERNAME}}',
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{Dumee_USER_EMAIL}}',
      'User-Username': '{{Dumee_USER_USERNAME}}',
      'Non-Existent': '{{Dumee_USER_NONEXISTENT}}',
    };
    ...
    expect(result).toEqual({
      ...
      'Non-Existent': '{{Dumee_USER_NONEXISTENT}}', // Unchanged for non-existent field
    });
  });
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      Dumee_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{Dumee_USER_EMAIL}}',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{Dumee_USER_ID}}',
      'X-User-Name': '{{Dumee_USER_NAME}}',
      'X-User-Username': '{{Dumee_USER_USERNAME}}',
      'X-User-Email': '{{Dumee_USER_EMAIL}}',
      'X-User-Provider': '{{Dumee_USER_PROVIDER}}',
      'X-User-Role': '{{Dumee_USER_ROLE}}',
      'X-User-GoogleId': '{{Dumee_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{Dumee_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{Dumee_USER_OPENIDID}}',
      'X-User-SamlId': '{{Dumee_USER_SAMLID}}',
      'X-User-LdapId': '{{Dumee_USER_LDAPID}}',
      'X-User-GithubId': '{{Dumee_USER_GITHUBID}}',
      'X-User-DiscordId': '{{Dumee_USER_DISCORDID}}',
      'X-User-AppleId': '{{Dumee_USER_APPLEID}}',
      'X-User-EmailVerified': '{{Dumee_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{Dumee_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{Dumee_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{Dumee_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{Dumee_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{Dumee_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{Dumee_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders(headers, user, customVars);
