...
const { EModelEndpoint, Constants, openAISettings } = require('dumee-data-provider');
...
describe('importDumeeConvo', () => {
  const jsonDataNonRecursiveBranches = JSON.parse(
    fs.readFileSync(path.join(__dirname, '__data__', 'Dumee-opts-nonr-branches.json'), 'utf8'),
  );
  ...
  it('should import conversation correctly', async () => {
    ...
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'Dumee-export.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
  ...
  it('should import linear, non-recursive thread correctly with correct endpoint', async () => {
    ...
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'Dumee-linear.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
  ...
  it('should maintain correct message hierarchy (tree parent/children relationship)', async () => {
    const jsonData = JSON.parse(
      fs.readFileSync(path.join(__dirname, '__data__', 'Dumee-tree.json'), 'utf8'),
    );
    const requestUserId = 'user-123';
    const importBatchBuilder = new ImportBatchBuilder(requestUserId);
    jest.spyOn(importBatchBuilder, 'saveMessage');
...
describe('processAssistantMessage', () => {
  const testMessage = 'This is a test citation ã€3:0â€ sourceã€‘ã€3:1â€ sourceã€‘';

  const messageData = {
    metadata: {
      citations: [
        {
          start_ix: 23, // Position of first "ã€3:0â€ sourceã€‘"
        ...
        {
          start_ix: 36, // Position of second "ã€3:1â€ sourceã€‘"
  ...
  test('should handle potential ReDoS attack payloads', () => {
    ...
    sizes.forEach((size) => {
      ...
      const maliciousText = 'ã€' + 'a'.repeat(size) + 'ã€‘';
