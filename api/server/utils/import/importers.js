...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('dumee-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Dumee conversation');
    return importDumeeConvo;
  }
...
/**
 * Imports a Dumee conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importDumeeConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Dumee file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Dumee file`, error);
  }
}
