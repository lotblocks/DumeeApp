const { webSearchKeys } = require('@dumee/api');
const {
  Constants,
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `dumee.yaml` config file instead.\nMore info: https://dum.ee/docs/configuration/balance',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://dum.ee/docs/configuration/credentials
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://dum.ee/docs/configuration/authentication
  ...
  webSearchKeys.forEach((key) => {
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          `â— Web search configuration error: ${key} contains an actual value instead of an environment variable reference.
          
          Current value: "${value.substring(0, 10)}..."
          
          This is incorrect! You should use environment variable references in your dumee.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://dum.ee/docs/configuration/web_search`,
        );
      }
    }
  });
