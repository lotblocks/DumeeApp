// Mock dumee-data-provider
jest.mock('dumee-data-provider', () => ({
  ...jest.requireActual('dumee-data-provider'),
...
const { extractVariableName } = require('dumee-data-provider');
...
describe('checkWebSearchConfig', () => {
  ...
  describe('when config values are actual values instead of environment variable references', () => {
    it('should warn when serperApiKey contains actual API key', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'â— Web search configuration error: serperApiKey contains an actual value',
        ),
      );
      expect(logger.warn).toHaveBeenCalledWith(
    ...
    it('should warn when firecrawlApiUrl contains actual URL', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'â— Web search configuration error: firecrawlApiUrl contains an actual value',
        ),
      );
      expect(logger.warn).toHaveBeenCalledWith(
    ...
    it('should include documentation link in warning message', () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining(
          'More info: https://www.Dumee.ai/docs/configuration/Dumee_yaml/web_search',
        ),
      );
    });
  });
