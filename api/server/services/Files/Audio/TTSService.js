...
const { genAzureEndpoint } = require('@dumee/api');
const { extractEnvVariable, TTSProviders } = require('dumee-data-provider');
...
class TTSService {
  ...
  getProvider() {
    ...
    if (!ttsSchema) {
      throw new Error(
        'No TTS schema is set. Did you configure TTS in the custom config (Dumee.yaml)?',
      );
    }
