...
const sharp = (input) => ({
  metadata: () => Promise.resolve({ width: 1024, height: 1024, format: 'jpeg' }),
  resize: (width, height, options) => ({ toBuffer: () => Promise.resolve(Buffer.from(input || '')) }),
...
const { EImageOutputType } = require('dumee-data-provider');
