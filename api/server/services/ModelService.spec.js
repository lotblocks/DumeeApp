...
const { EModelEndpoint, defaultModels } = require('dumee-data-provider');
...
jest.mock('@dumee/data-schemas', () => ({
  ...jest.requireActual('@dumee/data-schemas'),
  logger: {
    error: jest.fn(),
  },
}));
...
describe('getOpenAIModels with mocked config', () => {
  it('uses alternative behavior when userProvidedOpenAI is true', async () => {
    ...
    jest.mock('dumee-data-provider', () => {
      const original = jest.requireActual('dumee-data-provider');
      return {
        ...original,
        defaultModels: {
