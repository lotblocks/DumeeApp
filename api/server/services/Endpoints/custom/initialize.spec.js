...
jest.mock('~/server/services/Config', () => ({
  getCustomEndpointConfig: jest.fn().mockResolvedValue({
    ...
    headers: { 'x-user': '{{Dumee_USER_ID}}', 'x-email': '{{Dumee_USER_EMAIL}}' },
...
describe('custom/initializeClient', () => {
  ...
  it('calls resolveHeaders with headers and user', async () => {
    const { resolveHeaders } = require('@dumee/api');
    await initializeClient({ req: mockRequest, res: mockResponse, optionsOnly: true });
    expect(resolveHeaders).toHaveBeenCalledWith(
      { 'x-user': '{{Dumee_USER_ID}}', 'x-email': '{{Dumee_USER_EMAIL}}' },
