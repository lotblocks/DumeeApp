...
jest.mock('@dumee/api', () => ({
  ...jest.requireActual('@dumee/api'),
  isEnabled: jest.fn(() => false),
}));
jest.mock('@dumee/data-schemas', () => ({
  ...jest.requireActual('@dumee/api'),
  logger: {
    info: jest.fn(),
    debug: jest.fn(),
...
describe('setupOpenId', () => {
  ...
  it('should create a new user with correct username when username claim exists', async () => {
    // Arrange â€“ our userinfo already has username 'flast'
  ...
  it('should use given_name as username when username claim is missing', async () => {
    // Arrange â€“ remove username from userinfo
    const userinfo = { ...tokenset.claims() };
  ...
  it('should use email as username when username and given_name are missing', async () => {
    // Arrange â€“ remove username and given_name
    const userinfo = { ...tokenset.claims() };
    delete userinfo.username;
  ...
  it('should override username with OPENID_USERNAME_CLAIM when set', async () => {
    // Arrange â€“ set OPENID_USERNAME_CLAIM so that the sub claim is used
    ...
    // Assert â€“ username should equal the sub (converted as-is)
  ...
  it('should override full name with OPENID_NAME_CLAIM when set', async () => {
    // Arrange â€“ use the name claim as the full name
  ...
  it('should update an existing user on login', async () => {
    // Arrange â€“ simulate that a user already exists
    const existingUser = {
      _id: 'existingUserId',
      provider: 'local',
    ...
    // Assert â€“ updateUser should be called and the user object updated
  ...
  it('should enforce the required role and reject login if missing', async () => {
    // Arrange â€“ simulate a token without the required role.
    jwtDecode.mockReturnValue({
      roles: ['SomeOtherRole'],
    });
    const userinfo = tokenset.claims();
    ...
    // Assert â€“ verify that the strategy rejects login
    expect(user).toBe(false);
    expect(details.message).toBe('You must have the "requiredRole" role to log in.');
  });

  it('should attempt to download and save the avatar if picture is provided', async () => {
    // Arrange â€“ ensure userinfo contains a picture URL
    const userinfo = tokenset.claims();

    // Act
    const { user } = await validate(tokenset);

    // Assert â€“ verify that download was attempted and the avatar field was set via updateUser
  ...
  it('should not attempt to download avatar if picture is not provided', async () => {
    ...
    // Assert â€“ fetch should not be called and avatar should remain undefined or empty
